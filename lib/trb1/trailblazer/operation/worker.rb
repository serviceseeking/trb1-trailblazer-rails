...
class Trb1::Trailblazer::Operation
      ...
      module ClassMethods
        def file_marshaller_representer
          @file_marshaller_representer ||= contract_class.schema(include: [Representable::Hash]).apply do |dfn|
            ...
            dfn.merge!(
              serialize: lambda { |file, *| Trb1::Trailblazer::Operation::UploadedFile.new(file, tmp_dir: "/tmp/uploads").to_hash },
              deserialize: lambda { |object, hash, *| Trb1::Trailblazer::Operation::UploadedFile.from_hash(hash) },
              class: Hash
            )
          end
        end
